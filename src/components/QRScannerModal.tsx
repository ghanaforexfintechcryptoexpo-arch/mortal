        ...
        <div className="p-6 flex flex-col md:flex-row gap-6 items-stretch justify-between">
                    ...
                    <div className="p-6 text-center space-y-4 max-w-xs">
                      {hasCamera === false ? (
                        <>
                          ...
                          <div>
                            <h4 className="text-xs font-mono text-amber-400 font-bold uppercase">Webcam Access Blocked</h4>
                            <p className="text-[10px] text-gray-400 mt-1">Camera is restricted. Please select a physical diagnostic scan simulator code on the right.</p>
                          </div>
                        </>
                      ) : (
                        <>
                          <RefreshCw className="w-8 h-8 text-emerald-400 animate-spin mx-auto" />
                          <p className="text-[10px] text-gray-400 font-mono">Requesting client camera system stream...</p>
                        </>
                      )}
                    </div>
                  )}
                ...
                <div className="p-6 text-center space-y-4 flex flex-col items-center justify-center h-full w-full bg-emerald-950/10">
                  ...
                  {scanResult?.product && (
                    <div className="flex items-center gap-3 p-2 bg-black/40 border border-gray-800 rounded-lg w-full max-w-xs text-left">
                      <img src={scanResult.product.image} alt={scanResult.product.name} className="w-10 h-10 object-contain" />
          ...
          <div className="w-full md:w-64 flex flex-col space-y-3.5 border-t md:border-t-0 md:border-l border-gray-800 pt-5 md:pt-0 md:pl-5">
            ...
            <div className="border-t border-gray-800 pt-3.5 space-y-2">
              <span className="text-[10px] font-mono font-bold text-gray-500 uppercase tracking-wider block">Manual shelf tag override</span>
              <div className="flex gap-1.5">
                <input
                  type="text"
                  placeholder="Enter QR/shelf code..."
                  value={manualCode}
                ...
                <button
                  ...
                  onClick={() => {
                    ...
                    // Find any product matching
                    const matchedProd = products.find(p => 
                      p.name.toLowerCase().includes(manualCode.toLowerCase()) || 
                      p.brand.toLowerCase().includes(manualCode.toLowerCase())
                    );
                    handleCodeDetected({
                      name: matchedProd ? matchedProd.name : `Tag "${manualCode}"`,
                      code: `MANUAL-${manualCode.toUpperCase()}`,
